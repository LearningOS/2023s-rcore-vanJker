...
impl TaskManager {
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            // if the tasks[next] is first time to run, set its start time
    ...
    /// Return the current 'Running' task's syscall times
    ...
    /// Return the current 'Running' task's start time
